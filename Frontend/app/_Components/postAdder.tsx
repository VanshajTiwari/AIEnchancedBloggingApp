...
export default function PostEditor() {
    ...
    const handleDragOver = (event: React.DragEvent, index: number) => {
        ...
        const [draggedItem] = updatedContent.splice(draggingIndex, 1); // Remove the dragging item
    ...
    return (
        <div className="w-full p-4 bg-white rounded-md shadow-md border-4 border-black duration-400">
            <div className="mb-4">
            ...
            {content.map((item: any, index: number) => (
                ...
                >
                    ...
                    {
                    item.con_type=="file"?<>
                        <label className="flex flex-col items-center justify-center pt-5 pb-6 hover:bg-white hover:cursor-pointer rounded-md duration-300" htmlFor="dropzone-file">
            ...
            <div className="mt-6">
                <button onClick={handleSubmit} className="bg-green-500 text-white p-2 rounded-md disabled:bg-gray-500 duration-300" disabled>Submit Post</button>
            </div>
        </div>
    );
}
